...
def get_grey_value(pixel, gamma=2):
    ...
    return abs(255 * int(WHITE_BACKGROUND) - pixel[0]) if len(pixel) == 1 else 0 if pixel[0] + pixel[1] + pixel[2] == 255+255+255 and WHITE_BACKGROUND else ((pixel[0]**gamma + pixel[1]**gamma + pixel[2]**gamma)/3)**(1/gamma) if len(pixel) == 3 else ((pixel[0]**gamma + pixel[1]**gamma + pixel[2]**gamma)/3)**(1/gamma) * pixel[3] / 255
