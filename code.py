...
PATH = "image.png"
CONTRAST_THRESHOLD_LOW = 96     # range (0 - 255) removes dark areas
CONTRAST_THRESHOLD_HIGH = 256   # range (0 - 255) removes light areas
GAMMA = 2
SPACE_CHARACTER = chr(int("2005", 16))*3
...
def convert_to_binary(image, threshold_low, threshold_high, gamma):
    """returns 2D array of 0's and 1's\n0 if pixel < threshold 1 otherwise"""

    return np.array([1 if threshold_low <= get_grey_value(image.getpixel((i, j)), gamma) <= threshold_high else 0 for j in range(image.size[1]) for i in range(image.size[0])]).reshape(image.size[1], image.size[0])
...
s = split_array(convert_to_binary(pil.open(PATH), CONTRAST_THRESHOLD_LOW, CONTRAST_THRESHOLD_HIGH, GAMMA), 2, 4)

# convert into characters and write to file
open("text.txt", 'w', encoding="utf-8").write(''.join(k for k in ['\n' if j == s.shape[1] else chr(int("2800", 16) + s[i][j][0][0]*1 + s[i][j][1][0]*2 + s[i][j][2][0]*4 + s[i][j][0][1]*8 + s[i][j][1][1]*16 + s[i][j][2][1]*32 + s[i][j][3][0]*64 + s[i][j][3][1]*128) if s[i][j][0][0]|s[i][j][1][0]|s[i][j][2][0]|s[i][j][0][1]|s[i][j][1][1]|s[i][j][2][1]|s[i][j][3][0]|s[i][j][3][1] != 0 else SPACE_CHARACTER for i in range(s.shape[0]) for j in range(s.shape[1] + 1)]))
